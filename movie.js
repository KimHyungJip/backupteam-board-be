...
const selectMovie = movies.filter(movie=>(movie.user_id ===1)).map(a=>a.movie_title);

//Looney Tunes: Back in Action 라는 movie_title 을 가진 게시물의 작성자의 이메일을 가져온다.
const userEmail = users.find(user=>user.id===movies.find(movie=>movie.movie_title==='Looney Tunes: Back in Action').user_id).email;
